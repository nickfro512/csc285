...
char menu_select_get(int menu_type)
{
	...
	case 1:
		...
		cout << "MEDIA OPTIONS: \n";
		cout << "(m) Add media item" << endl;
		cout << "(x) Delete media item" << endl;
		cout << "(t) Edit media item" << endl;
		cout << "(i) List media items" << endl;
		break;
	case 2:
		cout << "PATRON MENU: " << endl;
		cout << "(e) Edit profile" << endl;
		cout << "(s) Search media" << endl;
		cout << "(o) Check out media" << endl;
		cout << "(i) Check in media" << endl;
		cout << "(l) View checked out list" << endl;
		break;
	}
	cout << "(t) Toggle Login Menu" << endl;
	cout << "(x) exit" << endl;
	cin >> command;
...
int main()
{
	...
	*/
	...
	while (menu_command != 'x')
	{
		...
		if (menu_command == 't')	// reverts back to login menu
				menu_select = 0;

		if (menu_select == 0)	// login menu
		{
			string tempUsername;
			string tempPassword;
			...
			switch (menu_command)
			{
			case 'a':	// creates an account for user/admin if they do not already have one
				int newUserID;
				theUser = menu_user_add();
				newUserID = theUserHandler.addUser(theUser);
				cout << "Successfully added user ID " << newUserID << endl;
				break;
			case 'l':	// logs a user in as patron or admin if they already have an account
				cout << "Username: ";
				cin >> tempUsername;
				cout << "\nPassword: ";
				cin >> tempPassword;
				if (tempUsername == theUser.username && tempPassword == theUser.password)	// if username/password is correct, will log in as patron or admin depending on userType
				{
					cout << "\nWelcome " << theUser.firstName << " " << theUser.lastName << endl;
					if (theUser.userType == 'a')
						menu_select = 1;	// logs user in as admin
					else
				...
				break;
			default:	// in case of incorrect menu_command
		...
		else if(menu_select = 1)	// admin menu
		{
			Media editedMedia;
			User editedUser;
			...
			switch (menu_command)
			{
			case 'a':	// allows admin to add a new user
				int newUserID;
				theUser = menu_user_add();
				newUserID = theUserHandler.addUser(theUser);
				cout << "Successfully added user ID " << newUserID << endl;
